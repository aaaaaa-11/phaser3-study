...
let head: CustomImage // 头
let tail: CustomImage // 尾
...
function updateCustomKey() {
  const children = snake.getChildren() as CustomImage[]
  children.forEach((child, i) => {
    child.customKey = formState.direction ? children.length - i - 1 : i
  })
}
...
function eatFood(obj1: any, obj2: any) {
  ...
  if (!formState.direction) {
    ...
    // 把原来的整个往后挪一格、移除碰撞
    const snakeChildren = snake.getChildren() as CustomImage[]
